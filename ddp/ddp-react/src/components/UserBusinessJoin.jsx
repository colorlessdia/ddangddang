...
const UserBusinessJoin = () =>{
    ...
    //사업자 등록번호 검사
    const [userBizNum, setUserBizNum] = useState("");
    const [userBizNumError, setUserBizNumError] = useState("");

    const handleUserBizNumChange = (event) => {
        const value = event.target.value;
        setUserBizNum(value);
        
        if (value.length == 0) {
            setUserBizNumError("사업자등록번호를 입력해주세요.");
        } else {
            setUserBizNumError("");
        }
    };
        ...
        <div className="wrap">
            <div className="login">
                <h2>회원가입(사업자)</h2>
                    ...
                    {/* <!-- 사업자등록번호 --> */}
                    <input type="text" id="userBizNum" name="BIZ_NUM" placeholder="사업자등록번호번호" maxlength="12"  value={userBizNum} onChange={handleUserBizNumChange} required/>
                    {userBizNumError && <p className="error">{userBizNumError}</p>}
...
export default UserBusinessJoin;
